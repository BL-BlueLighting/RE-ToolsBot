...
from toolsbot.plugins.userInfoController import *
...
async def _ (bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if cmd == "fetch" or cmd != "add":
        ...
        # if table not exists create it
        cursor.execute("CREATE TABLE IF NOT EXISTS saynormal (keyword TEXT, content TEXT, ban tinyint(1)); ")
        cursor.execute("SELECT * FROM saynormal WHERE keyword = ?", (content.split(" ") [0], ))
        if len(cursor.fetchall()) != 0:
            cursor.execute("INSERT INTO saynormal (keyword, content, ban) VALUES (?, ?, ?)", (content.split(" ") [0], content.split(" ") [1:], 0))
            msg += "\n    - 添加成功。"
            await saynormal_function.finish(msg)
        msg += "\n    - 关键词不存在。"
        await saynormal_function.finish(msg)
